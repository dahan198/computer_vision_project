...
def func_eval(recog_path, file_list):
    ground_truth_path = "../APAS/transcriptions_gestures/"
    mapping_file = "../APAS/mapping_gestures.txt"
    list_of_videos = file_list
    ...
    for vid in list_of_videos:
        ...
        c = []
        for con in gt_content:
            content_list = con.split(' ')
            c.extend([content_list[2]] * (1 + int(content_list[1]) - int(content_list[0])))
        gt_content = c
...
def main():
    ...
    if args.split == 0:
        for split in range(splits):
            test_files = set(map(lambda x: x.split('.csv')[0],
                                 (open("../APAS/folds/test " + str(split) + ".txt").readlines())))
            recog_path = "./{}/".format(args.result_dir) + "/split_{}".format(split) + "/"
            acc, edit, f1s = func_eval(recog_path, test_files)
            acc_all += acc
            edit_all += edit
            f1s_all[0] += f1s[0]
            f1s_all[1] += f1s[1]
        ...
        acc_all /= splits
        edit_all /= splits
        f1s_all = [i / splits for i in f1s_all]
    else:
        split = args.split
        recog_path = "./{}/".format(args.result_dir) + args.dataset + "/split_{}".format(split) + "/"
        acc_all, edit_all, f1s_all = func_eval(args.dataset, recog_path)
